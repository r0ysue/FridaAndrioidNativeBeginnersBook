...
Java_com_example_opensslmd5_MainActivity_stringFromJNI(
        JNIEnv* env,
        jobject,
        jstring str) {
    // 获取Java层传入的明文
    const char *stext = env->GetStringUTFChars(str, JNI_FALSE);
    ...
    // 传入待处理的内容以及内容的长度
    MD5_Update(&context, stext, strlen(stext));
    ...
    //  根据NDK开发规范，GET或者NEW的资源必须要释放掉
    env->ReleaseStringChars(str, reinterpret_cast<const jchar *>(stext));
    // 根据 OPENSSL 标准，运算结束后清除上下文的结构体，以防止针对内存相关的攻击
    OPENSSL_cleanse(&stext,sizeof(stext));
