...
static void test2(unsigned int yy, uint64_t add) {
    ...
    if (count) {
        ...
        if (strcmp(insn[0].mnemonic, "b") == 0) {
            isb_in = 1;
            strtok(code, "x");
            address_in = strtoul(strtok(NULL, ""), NULL, 16);
            ...
            strtok(NULL, "x");
            address_in = strtoul(strtok(NULL, ""), NULL, 16);
            __android_log_print(6, "r0ysue", "%p", address_in);
        }
...
void register_hook(void *address1, void *func_replace) {
    ...
    void *a = malloc(0x20c);
    memcpy(a, (void *) myreplace, 0x10c);
    __android_log_print(6, "r0ysue", " aaaaaaaaaa %p", a);
...
//    mprotect((void*)PAGE_START((long)myreplace),PAGE_SIZE,PROT_WRITE|PROT_READ|PROT_EXEC);
    ...
    *(long *) ((char *) (a) + n + 12) = reinterpret_cast<long>(func_replace);
    ...
    for (int n = 0x9c; n <= 0x9c + 72; n = n + 24) {
        ...
        test2(*(int *) ((char *) addr + (n - 0x9c) / 6),
              reinterpret_cast<uint64_t>((char *) addr + (n - 0x9c) / 6));
        if (isb_in == 1) {

            if ((unsigned long) address_in - (unsigned long) addr <= 0x10) {
                address_in = ((unsigned long) huifu +
                           ((unsigned long) address_in - (unsigned long) addr) * 6) +
                          (unsigned long) 0x9c;
            ...
            *(long *) ((char *) (huifu) + n + 8) = address_in;
//        00021FD6
        } else if (isbl_in == 1) {
...
//        00023FD6
            ...
            *(long *) ((char *) (huifu) + n + 12) = address_in;
            ...
            *(int *) ((char *) (huifu) + n) =
                    *(int *) ((char *) addr + (n - 0x9c) / 6) & 0xff00000f;
...
//        00023FD6
            ...
            *(long *) ((char *) (huifu) + n + 16) = address_in;
            ...
            if ((unsigned long) address_in - (unsigned long) addr <= 0x10) {
                address_in = ((unsigned long) huifu +
                           ((unsigned long) address_in - (unsigned long) addr) * 6) +
                          (unsigned long) 0x9c;
                __android_log_print(6, "r0ysue", "%p %p %p", address_in, addr, huifu);
            }

            *(int *) ((char *) (huifu) + n) =
                    *(int *) ((char *) addr + (n - 0x9c) / 6) & 0xffff000f;
            *(int *) ((char *) (huifu) + n) = *(int *) ((char *) (huifu) + n) ^ 0x40;
            *(int *) ((char *) (huifu) + n + 4) = 0x14000005;
//        00023FD6
            *(int *) ((char *) (huifu) + n + 8) = 0x58000050;
